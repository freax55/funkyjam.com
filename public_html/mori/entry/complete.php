...
class Action extends DatabaseAction {
	...
	function prepare() {
		$this->_subject = '森大輔オフィシャルファンクラブ「モリノナカマ」本登録完了しました';
	...
	function execute() {
		...
		$result = $db->statement('mori/entry/sql/order_list_insert.sql');
		 ...
		 * order_descへの登録処理
		 */
		$result = $db->statement('mori/entry/sql/order_desc_list_insert.sql');
		...
		//会員番号
		$account_no = "M";
		$account_no .= date("ym", mktime(0, 0, 0, date("n"), date("j"), date("Y")));
		$account_no .= substr(mt_rand(10000, 19999), 1, 4);

		//会員番号が既に無いかチェック
		$result = $db->statement('mori/entry/sql/account_no_check.sql');
		...
		while(in_array($account_no, $new_arr)){
			$account_no = "";
			$account_no = "M";
			$account_no .= date("ym", mktime(0, 0, 0, date("n"), date("j"), date("Y")));
			$account_no .= substr(mt_rand(10000, 19999), 1, 4);
		...
		//メール開始日3
		$mail_stamp3 = date("Y-m-d", mktime(0, 0, 0, date("n"), date("j")-7, date("Y")+1));

		$db->begin();
		$db->assign('account_no', $account_no);
...
// if($this->status == "edit"){
// 	$db->statement('premium/entry/sql/update_premium.sql');
// }
		//登録処理
		$db->statement('mori/entry/sql/mori_user_insert.sql');
		...
		$mailer->assign('account_no', $account_no);
		...
		if ($row['payment'] == "コンビニ決済") {
			$this->_subject = "「モリノナカマ」入会金・年会費お支払いに関しまして（コンビニ決済）";
		}
		if ($this->mail) {
		...
		if ($row['payment'] == "コンビニ決済") {
			...
			$body = $mailer->fetch('mori/entry/mail/mail_paygent_cvs_customer.html');
			...
			if ($this->mail) {
				...
				$mailer->assign('admin', 0);
				$mailer->assign('noTourGoodsList', $noTourGoodsList);
				$body = $mailer->fetch('mori/entry/mail/mail_paygent_cvs.html');
				$body = $this->convertEncodingBody($body);
			 ...
			 */
			$db->begin();
			$db->statement('mori/entry/sql/update_apply_date_order_desc.sql');
			$db->commit();
		}
		elseif ($row['payment'] == "カード決済（ペイジェント）") {
			...
			$mailer->assign('noTourGoodsList', $noTourGoodsList);
			$body = $mailer->fetch('mori/entry/mail/mail_paygent_card_customer.html');
			$body = $this->convertEncodingBody($body);
			$this->send($system, $this->convertEncodingHeader('【'. $row["name"].'　様の'.$this->_subject . '】'), $body, $customer);
			...
			if ($this->mail) {
				...
				$mailer->assign('admin', 0);
				$mailer->assign('noTourGoodsList', $noTourGoodsList);
				$body = $mailer->fetch('mori/entry/mail/mail_paygent_card.html');
				$body = $this->convertEncodingBody($body);
				$this->send($customer, $subject, $body, $system);
			}
			...
			foreach($orderList as $orderRow) {
				...
				$db->statement('mori/entry/sql/update_order.sql');
			}
			$db->statement('mori/entry/sql/update_order_desc.sql');
			$db->commit();
			...
			// if($this->status == "edit"){
			// 	$db->begin();
			// 	$db->statement('premium/entry/sql/update_premium_card_user.sql');
			// 	$db->commit();
			// }
			/*
			 * apply_dateを更新する
