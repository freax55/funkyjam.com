...
class Action extends ShopAction {
	...
	function init() {
		...
		define('RET_URL', '/insert/index.php?action=complete&');
	...
	function execute() {
		...
		if($this->payment == 'コンビニ決済') {
			$cvs_price = 250;
			$this->itemList["yearsPASS"]["price"] = $this->itemList["yearsPASS"]["price"]+$cvs_price;
		}
		//DB登録処理
	...
	function validate() {
		...
		if(empty($this->convenience_store_no) && $this->payment == 'コンビニ決済') {
			...
			if(!preg_match('/^(?:4[0-9]{12}(?:[0-9]{3})?|5[1-5][0-9]{14}|6011[0-9]{12}|3(?:0[0-5]|[68][0-9])[0-9]{11}|3[47][0-9]{13}|(?:2131|1800|35[0-9]{3})[0-9]{11})$/', "{$this->card_no01}{$this->card_no02}{$this->card_no03}{$this->card_no04}")){
