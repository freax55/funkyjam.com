...
class Action extends AuthAction {
	...
	function execute() {
		...
		$result = $db->statement('artist/kubota/ticket_reserve/sql/order_list.sql');
		...
		//order_desc
		$result = $db->statement('artist/kubota/ticket_reserve/sql/order_desc_list.sql');
		...
		if($row['payment'] == "予約") {
			$subject = $this->convertEncodingHeader($this->_subject. '（BARI BARI CREW カード決済（予約））');
		}else{
		...
		if ($result_code == '0' && $detail_code == '00') {
			...
			$body = $mailer->fetch('artist/kubota/ticket_reserve/mail_card.html');
			...
			foreach($orderList as $orderRow) {
				...
				$db->statement('artist/kubota/ticket_reserve/sql/update_order.sql');
			}
			$db->statement('artist/kubota/ticket_reserve/sql/update_order_desc.sql');
			$db->commit();
			 ...
			 */
			$db->begin();
			$db->statement('artist/kubota/ticket_reserve/sql/update_apply_date_order_desc.sql');
			$db->commit();
		...
		elseif ($row['payment'] == "コンビニ決済") {
			...
			$body = $mailer->fetch('artist/kubota/ticket_reserve/mail_paygent_cvs.html');
			$body = $this->convertEncodingBody($body);
		...
		elseif ($row['payment'] == "ATM決済") {
			...
			//コンビニ・ATMについては、「予約」として、FJに通知する。
			$mailer->assign('admin', 0);
			$body = $mailer->fetch('artist/kubota/ticket_reserve/mail_paygent_atm.html');
			$body = $this->convertEncodingBody($body);
			$this->send($system, $this->convertEncodingHeader($this->_subject . '（予約）'), $body, $customer);
		...
		elseif ($row['payment'] == "予約") {
			...
			$body = $mailer->fetch('artist/kubota/ticket_reserve/mail_postal.html');
			$body = $this->convertEncodingBody($body);
			$this->send($system, $this->convertEncodingHeader($this->_subject . '（BARI BARI CREW カード決済（予約））'), $body, $customer);
