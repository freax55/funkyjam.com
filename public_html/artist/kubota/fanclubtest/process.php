...
class ProcessAction extends Action {
	function execute() {
		...
		$mail->setBody( dirname(__FILE__).'/' .$m['customer']['body'] , $this->getProperties(),'UTF-8' );
		...
		$mail->setSubject($m['customer']['subject_ticket']);
		$mail->setBody( dirname(__FILE__).'/' .$m['customer']['body_ticket'] , $this->getProperties(),'UTF-8' );
		$mail->setTo($this->form['mail'],$this->form['mail']);
		$mail->send();
