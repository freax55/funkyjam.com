...
class AbstractController extends Debug {
	...
	//var $__systemCharset = 'EUC-JP';
	var $__systemCharset = 'UTF-8';
	var $__templateCharset = 'SJIS';
	...
	*/
	function run() {
		...
		if($this->__systemCharset != $this->__templateCharset) {
			...
			if (preg_match('/\/contact|magazine|fanletter|fanclub|fanclubtest\//',$_SERVER['REQUEST_URI'])) {
		mb_convert_variables($this->__systemCharset, 'UTF-8', $_REQUEST);
			}
			else{
				mb_convert_variables($this->__systemCharset, $this->__templateCharset, $_REQUEST);
			}
			...
			if ($this->__isMobile) {
				array_walk($_REQUEST, array(&$this, 'convertMobile'));
			}
		}
	...
	*/
	...
	/**
	* Convert kana for mobile.
	* @access public
	*/
	function convertMobile(&$value, $key) {
		if (is_array($value)) {
			array_walk($value, array(&$this, 'convertMobile'));
		}
		else {
